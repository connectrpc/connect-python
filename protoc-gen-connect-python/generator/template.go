...
# source: {{.FileName}}
{{if .Services}}
from collections.abc import AsyncGenerator, AsyncIterator, Iterable, Iterator, Mapping, Sequence
...
class {{.Name}}ASGIApplication(ConnectASGIApplication[{{.Name}}]):
    def __init__(self, service: {{.Name}} | AsyncGenerator[{{.Name}}], *, interceptors: Iterable[Interceptor]=(), read_max_bytes: int | None = None, compressions: Sequence[Compression] | None = None) -> None:
        super().__init__(
            service=service,
...
class {{.Name}}WSGIApplication(ConnectWSGIApplication):
    def __init__(self, service: {{.Name}}Sync, interceptors: Iterable[InterceptorSync]=(), read_max_bytes: int | None = None, compressions: Sequence[Compression] | None = None) -> None:
        super().__init__(
